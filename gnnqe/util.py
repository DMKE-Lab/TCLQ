...
def create_working_directory(cfg):
    ...
    # working_dir = os.path.join(os.path.expanduser(cfg.output_dir),
    #                            cfg.task["class"], cfg.dataset["class"], cfg.task.model["class"],
    #                            time.strftime("2023-03-14-22-03-59"))
...
def build_solver(cfg, dataset):
    ...
    cfg.task.model.model.num_timestamp = dataset.num_timestamp
    # 初始化模型
    task = core.Configurable.load_config_dict(cfg.task)
    cfg.optimizer.params = task.parameters()
    optimizer = core.Configurable.load_config_dict(cfg.optimizer)
    """
    处理有关训练和任务测试的所有内容的普通类。
    此类可以在多个CPU或GPU上执行同步分布式并行训练。要调用并行训练，请使用以下命令之一启动。
    """
