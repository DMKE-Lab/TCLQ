...
class TemporalGraph(data.Graph):
    r"""
    ...
    _meta_types = {"node", "edge", "timestamp", "graph", "node reference", "edge reference", "timestamp reference",
                   "graph reference"}

    def __init__(self, edge_list=None, edge_weight=None, num_node=None, num_relation=None,
                 num_timestamp=None, node_feature=None, edge_feature=None, timestamp_feature=None, graph_feature=None,
                 timestamp_vocab=None, tid_vocab=None,
                 **kwargs):
        super(TemporalGraph, self).__init__(**kwargs)
        ...
        if timestamp_feature is not None:
            self.timestamp_feature = torch.as_tensor(timestamp_feature, device=self.device)
    ...
    def timestamp_reference(self):
        """
        Context manager for timestamp references.
        """
        return self.context("timestamp reference")
    ...
    def set_timestamp_vocab(self, timestamp_vocab):
        self.timestamp_vocab = timestamp_vocab

    def set_tid_vocab(self, tid_vocab):
        self.tid_vocab = tid_vocab
    ...
    def _check_attribute(self, key, value):
        for type in self._meta_contexts:
            ...
            if type == "node":
                ...
                if len(value) != self.num_timestamp:
                    raise ValueError("Expect timestamp attribute `%s` to have shape (%d, *), but found %s" %
                                     (key, self.num_timestamp, value.shape))
            ...
            elif type == "timestamp reference":
                is_valid = (value >= -1) & (value < self.num_timestamp)
                if not is_valid.all():
                    error_value = value[~is_valid]
                    raise ValueError("Expect edge reference in [-1, %d), but found %d" %
                                     (self.num_timestamp, error_value[0]))
    ...
    def __setattr__(self, key, value):
        ...
        super(TemporalGraph, self).__setattr__(key, value)
    ...
    def _standarize_edge_list(self, edge_list, num_relation):
        if edge_list is not None and len(edge_list):
            ...
            num_element = 2 if num_relation is None else 4
    ...
    def _standarize_timestamp_weight(self, timestamp_weight, edge_list):
        if timestamp_weight is not None:
            timestamp_weight = torch.as_tensor(timestamp_weight, dtype=torch.float, device=edge_list.device)
            if len(edge_list) != len(timestamp_weight):
                raise ValueError("`edge_list` and `edge_weight` should be the same size, but found %d and %d"
                                 % (len(edge_list), len(timestamp_weight)))
        else:
            timestamp_weight = torch.ones(len(edge_list), device=edge_list.device)
    ...
    def _standarize_num_relation(self, num_relation, edge_list):
        ...
        if num_relation is not None:
            ...
            if edge_list.shape[1] <= 2:
                ...
                raise ValueError("`num_relation` is %d, but found relation %d in `edge_list`" % (
                    num_relation, edge_list[:, 2].max()))
        return num_relation

    def _standarize_num_timestamp(self, num_timestamp, edge_list):
        if num_timestamp is None and edge_list.shape[1] > 2:
            num_timestamp = self._maybe_num_timestamp(edge_list)
        if num_timestamp is not None:
            num_timestamp = torch.as_tensor(num_timestamp, device=edge_list.device)
            min_timestamp = edge_list[:, 3].min().item()
            if edge_list.shape[1] <= 2:
                raise ValueError("`num_relation` is provided, but the number of dims of `edge_list` is less than 3.")
            # elif ((edge_list[:, 3] - min_timestamp) >= num_timestamp).any():
            elif (len(set(edge_list[:, 3].cpu().numpy())) > num_timestamp).any():
                raise ValueError("`num_relation` is %d, but found relation %d in `edge_list`" % (
                    num_timestamp, edge_list[:, 3].max()))
    ...
    def _maybe_num_timestamp(self, edge_list):
        warnings.warn("_maybe_num_timestamp() is used to determine the number of timestamps. "
                      "This may underestimate the count if there are unseen timestamps.")
        # return edge_list[:, 3].max().item() - edge_list[:, 3].min().item() + 1
        return len(set(edge_list[:, 3].cpu().numpy()))
        # time = edge_list[:, 3]
        # return len(set(time.numpy()))
    ...
    @classmethod
    def from_dense(cls, adjacency, node_feature=None, edge_feature=None, timestamp_feature=None):
        """
        ...
        # timestamp_weight = adjacency[tuple(edge_list.t())]
        num_node = adjacency.shape[0]
        num_relation = adjacency.shape[2] if adjacency.ndim > 2 else None
        num_timestamp = adjacency.shape[3] if adjacency.ndim > 3 else None
        if edge_feature is not None:
            edge_feature = torch.as_tensor(edge_feature)
            edge_feature = edge_feature[tuple(edge_list.t())]
        if timestamp_feature is not None:
            timestamp_feature = torch.as_tensor(timestamp_feature)
            timestamp_feature = timestamp_feature[tuple(edge_list.t())]

        return cls(edge_list, edge_weight, num_node, num_relation, num_timestamp, node_feature,
                   edge_feature, timestamp_feature)
    ...
    def split(self, node2graph):
        ...
        return self.packed_type(edge_list[edge_index], edge_weight=self.edge_weight[edge_index], num_nodes=num_nodes,
                                ...
                                meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab, tid_vocab=self.tid_vocab,
                                **data_dict)
    ...
    def pack(cls, graphs):
        ...
        for graph in graphs:
            ...
            # timestamp_weight.append(graph.timestamp_weight)
            ...
            if num_relation == -1:
                ...
                raise ValueError("Inconsistent `num_relation` in graphs. Expect %d but got %d."
                                 % (num_relation, graph.num_relation))
            if num_timestamp == -1:
                num_timestamp = graph.num_timestamp
            elif num_timestamp != graph.num_timestamp:
                raise ValueError("Inconsistent `num_timestamp` in graphs. Expect %d but got %d."
                                 % (num_timestamp, graph.num_timestamp))
        ...
        timestamp_vocab = None

        return cls.packed_type(edge_list, edge_weight=edge_weight, num_nodes=num_nodes, num_edges=num_edges,
                               num_relation=num_relation, num_timestamp=num_timestamp, meta_dict=meta_dict,
                               timestamp_vocab=timestamp_vocab, **data_dict)
    ...
    def repeat(self, count):
        ...
        return self.packed_type(edge_list, edge_weight=edge_weight, num_nodes=num_nodes, num_edges=num_edges,
                                num_relation=num_relation, num_timestamp=num_timestamp, meta_dict=self.meta_dict,
                                timestamp_vocab=self.timestamp_vocab, **data_dict)
    ...
    def match(self, pattern, is_projection=False):
        """
        ...
        for i, mask in enumerate(mask_set):
            ...
            if query_type not in self.edge_inverted_index:
                if is_projection:
                    self.edge_inverted_index[query_type] = self._build_edge_inverted_index_time(mask)
                else:
    ...
    def _build_edge_inverted_index(self, mask):
        ...
        base = base[mask]
        max = reduce(int.__mul__, base.tolist())  # 256 * 304
    ...
    def _build_edge_inverted_index_time(self, mask):
        keys = self.edge_list[:, mask]
        base = torch.tensor(self.shape, device=self.device)
        base = base[mask]
        max = reduce(int.__mul__, base.tolist())  # 256 * 304
        ...
        # key = (keys * scale).sum(dim=-1)
        key = list()
        key.append(tuple(x if type(x) == list else x for x in (keys * scale)))
        order = key.argsort()
        num_keys = key.unique(return_counts=True)[1]
        ends = num_keys.cumsum(0)
        starts = ends - num_keys
    ...
    def node_mask(self, index, compact=False):
        ...
        return type(self)(edge_list[edge_index], edge_weight=self.edge_weight[edge_index], num_node=num_node,
                          num_relation=self.num_relation, num_timestamp=self.num_timestamp, meta_dict=meta_dict,
                          timestamp_vocab=self.timestamp_vocab, tid_vocab=self.tid_vocab, **data_dict)
    ...
    def edge_mask(self, index):
        ...
        # return TemporalGraph(self.edge_list[index], edge_weight=self.edge_weight[index], num_node=self.num_node,
        #                      num_relation=self.num_relation, num_timestamp=self.num_timestamp, meta_dict=meta_dict,
        #                      **data_dict)
    ...
    def line_graph(self):
        ...
        return TemporalGraph(edge_list, num_node=num_node, num_edge=num_edge, node_feature=node_feature)
    ...
    def full(self):
        ...
        if self.num_relation and self.num_timestamp:
            edge_list = torch.meshgrid(index, index, torch.arange(self.num_relation, device=self.device),
                                       torch.arange(self.num_timestamp, device=self.device))
        elif self.num_relation:
            edge_list = torch.meshgrid(index, index, torch.arange(self.num_relation, device=self.device))
        else:
        ...
        return type(self)(edge_list, edge_weight=edge_weight, num_node=self.num_node, num_relation=self.num_relation,
                          num_timestamp=self.num_timestamp, meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **data_dict)
    ...
    def undirected(self, add_inverse=False):
        ...
        num_relation = self.num_relation
        num_timestamp = self.num_timestamp
        if num_relation and add_inverse and num_timestamp:
            edge_list[:, 2] += num_relation
            num_relation = num_relation * 2
            edge_list[:, 3] += num_timestamp
            num_timestamp = num_timestamp * 2
        elif num_relation and add_inverse:
            edge_list[:, 2] += num_relation
            num_relation = num_relation * 2
    ...
    def adjacency_low(self):
        ...
        Otherwise, a sparse tensor of shape :math:`(|V|, |V|)` will be returned.
        """
        edge_list_t = self.edge_list.t()
        edge_weight = self.edge_weight
        shape = self.shape
        return utils.sparse_coo_tensor(self.edge_list.t(), self.edge_weight, self.shape)

    _tensor_names = ["edge_list", "edge_weight", "num_node", "num_relation", "num_timestamp", "edge_feature"]

    def to_tensors(self):
        edge_feature = getattr(self, "edge_feature", torch.tensor(0, device=self.device))
        return self.edge_list, self.edge_weight, self.num_node, self.num_relation, self.num_timestamp, edge_feature

    @classmethod
    def from_tensors(cls, tensors):
        edge_list, edge_weight, num_node, num_relation, num_timestamp, edge_feature = tensors
        if edge_feature.ndim == 0:
            edge_feature = None
        return cls(edge_list, edge_weight, num_node, num_relation, num_timestamp, edge_feature=edge_feature)
    ...
    def size(self, dim=None):
        if self.num_relation and self.num_timestamp:
            size = torch.Size((self.num_node, self.num_node, self.num_relation, self.num_timestamp))
        elif self.num_relation:
    ...
    def copy_(self, src):
        ...
        if self.num_timestamp is not None:
            self.num_timestamp.copy_(src.num_timestamp)
    ...
    def detach(self):
        ...
        Detach this graph.
        """
        return type(self)(self.edge_list.detach(), edge_weight=self.edge_weight.detach(), num_node=self.num_node,
                          num_relation=self.num_relation, num_timestamp=self.num_timestamp, meta_dict=self.meta_dict,
                          timestamp_vocab=self.timestamp_vocab, tid_vocab=self.tid_vocab,
    ...
    def clone(self):
        ...
        Clone this graph.
        """
        return type(self)(self.edge_list.clone(), edge_weight=self.edge_weight.clone(), num_node=self.num_node,
                          num_relation=self.num_relation, num_timestamp=self.num_timestamp, meta_dict=self.meta_dict,
                          timestamp_vocab=self.timestamp_vocab, tid_vocab=self.tid_vocab, **utils.clone(self.data_dict))
    ...
    def cuda(self, *args, **kwargs):
        ...
        if edge_list is self.edge_list:
            ...
            return type(self)(edge_list, edge_weight=self.edge_weight, num_node=self.num_node,
                              num_relation=self.num_relation, num_timestamp=self.num_timestamp,
                              meta_dict=self.meta_dict, timestamp_vocab=self.timestamp_vocab,
                              tid_vocab=self.tid_vocab, **utils.cuda(self.data_dict, *args, **kwargs))
    ...
    def cpu(self):
        ...
        if edge_list is self.edge_list:
            ...
            return type(self)(edge_list, edge_weight=self.edge_weight, num_node=self.num_node,
                              num_relation=self.num_relation, meta_dict=self.meta_dict,
                              num_timestamp=self.num_timestamp, timestamp_vocab=self.timestamp_vocab,
                              tid_vocab=self.tid_vocab, **utils.cpu(self.data_dict))
    ...
    def __repr__(self):
        ...
        if self.num_timestamp is not None:
            fields.append("num_timestamp=%d" % self.num_timestamp)
...
class PackedGraph(TemporalGraph):
    """
    ...
    def __init__(self, edge_list=None, edge_weight=None, num_nodes=None, num_edges=None, num_relation=None,
        ...
        super(PackedGraph, self).__init__(edge_list, edge_weight=edge_weight, num_node=num_node,
                                          num_relation=num_relation, num_timestamp=num_timestamp, **kwargs)
    ...
    def merge(self, graph2graph):
        ...
        return type(self)(graph.edge_list, edge_weight=graph.edge_weight, num_nodes=num_nodes,
                          num_edges=num_edges, num_relation=graph.num_relation, num_timestamp=graph.num_timestamp,
                          offsets=offsets, meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **data_dict)
    ...
    def repeat(self, count):
        ...
        return type(self)(edge_list, edge_weight=self.edge_weight.repeat(count),
                          ...
                          offsets=offsets, meta_dict=self.meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **data_dict)
    ...
    def repeat_interleave(self, repeats):
        ...
        return type(self)(edge_list, edge_weight=self.edge_weight[edge_index],
                          num_nodes=num_nodes, num_edges=num_edges, num_relation=self.num_relation,
                          num_timestamp=self.num_timestamp, offsets=offsets, meta_dict=self.meta_dict,
                          timestamp_vocab=self.timestamp_vocab, tid_vocab=self.tid_vocab, **data_dict)
    ...
    def get_item(self, index):
        ...
        return self.unpacked_type(edge_list, edge_weight=self.edge_weight[edge_index], num_node=self.num_nodes[index],
                                  num_relation=self.num_relation, num_timestamp=self.num_timestamp,
                                  meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab, **data_dict)
    ...
    def node_mask(self, index, compact=False):
        ...
        return type(self)(edge_list[edge_index], edge_weight=self.edge_weight[edge_index], num_nodes=num_nodes,
                          num_edges=num_edges, num_relation=self.num_relation, num_timestamp=self.num_timestamp,
                          offsets=offsets[edge_index], meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **data_dict)
    ...
    def edge_mask(self, index):
        ...
        # return TemporalGraph(self.edge_list[index], edge_weight=self.edge_weight[index], num_nodes=self.num_nodes,
        #                   num_edges=num_edges, num_relation=self.num_relation,num_timestamp=self.num_timestamp,
        #                   offsets=self._offsets[index], meta_dict=meta_dict, **data_dict)
        return type(self)(self.edge_list[index], edge_weight=self.edge_weight[index], num_nodes=self.num_nodes,
                          num_edges=num_edges, num_relation=self.num_relation, num_timestamp=self.num_timestamp,
                          offsets=self._offsets[index], meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **data_dict)
    ...
    def undirected(self, add_inverse=False):
        ...
        return type(self)(edge_list, edge_weight=self.edge_weight[index], num_nodes=self.num_nodes,
                          num_edges=self.num_edges * 2, num_relation=num_relation, num_timestamp=num_timestamp,
                          offsets=offsets, meta_dict=meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **data_dict)
    ...
    def detach(self):
        ...
        """
        return type(self)(self.edge_list.detach(), edge_weight=self.edge_weight.detach(), num_nodes=self.num_nodes,
                          num_edges=self.num_edges, num_relation=self.num_relation, num_timestamp=self.num_timestamp,
                          offsets=self._offsets, meta_dict=self.meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **utils.detach(self.data_dict))

    def clone(self):
        """
        Clone this packed graph.
        """
        return type(self)(self.edge_list.clone(), edge_weight=self.edge_weight.clone(), num_nodes=self.num_nodes,
                          num_edges=self.num_edges, num_relation=self.num_relation, num_timestamp=self.num_timestamp,
                          offsets=self._offsets, meta_dict=self.meta_dict, timestamp_vocab=self.timestamp_vocab,
                          tid_vocab=self.tid_vocab, **utils.clone(self.data_dict))
    ...
    def cuda(self, *args, **kwargs):
        ...
        if edge_list is self.edge_list:
            ...
            return type(self)(edge_list, edge_weight=self.edge_weight, num_nodes=self.num_nodes,
                              num_edges=self.num_edges, num_relation=self.num_relation,
                              num_timestamp=self.num_timestamp, offsets=self._offsets,
                              meta_dict=self.meta_dict, timestamp_vocab=self.timestamp_vocab,
                              tid_vocab=self.tid_vocab, **utils.cuda(self.data_dict, *args, **kwargs))
...
TemporalGraph.packed_type = PackedGraph
...
def cat(graphs):
    ...
    timestamp_vocab = graphs[0].num_timestamp
    num_timestamp = graphs[0].num_timestamp
    assert all(graph.num_relation == num_relation for graph in graphs)
    assert all(graph.num_timestamp == num_timestamp for graph in graphs)
    ...
    return type(graphs[0])(edge_list, edge_weight=edge_weight,
                           ...
                           num_timestamp=num_timestamp, offsets=offsets, meta_dict=meta_dict,
                           timestamp_vocab=timestamp_vocab, **data_dict)
