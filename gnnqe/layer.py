...
class GeneralizedRelationalConv(layers.MessagePassingBase):
    ...
    def __init__(self, input_dim, output_dim, num_relation, num_timestamp, query_input_dim, message_func="distmult",
        ...
        self.query_input_dim = query_input_dim  # 32
        ...
        if dependent:
            ...
            self.timestamp_linear = nn.Linear(query_input_dim, num_timestamp * input_dim)
            self.rel_time_linear = nn.Linear(query_input_dim * 2, num_relation * num_timestamp * self.input_dim)
        else:
            self.relation = nn.Embedding(num_relation, input_dim)
            self.timestamp = nn.Embedding(num_timestamp, input_dim)
            self.rel_time = nn.Embedding(num_relation * num_timestamp, input_dim)
    ...
    def message_and_aggregate(self, graph, input_r_t):
        ...
        input_r_t = input_r_t.flatten(1)
        boundary_r_t = graph.boundary_r_t.flatten(1)

        degree_out = graph.degree_out.unsqueeze(-1) + 1
        if self.dependent:
            rel_time_input = self.rel_time_linear(graph.query_r_t).view(batch_size, self.num_relation,
                                                                        self.num_timestamp, self.input_dim)
            rel_time_input = rel_time_input.transpose(0, 1).transpose(1, 2).flatten(2)
        else:

            rel_time_input = self.rel_time.weight.repeat(1, batch_size)
        ...
        if self.message_func in self.message2mul:
            ...
            raise ValueError("Unknown message function `%s`" % self.message_func)
        if self.aggregate_func == "sum":
            update_r_t = functional.generalized_rspmm(adjacency, rel_time_input, input_r_t, sum="add", mul=mul)
            update_r_t = update_r_t + boundary_r_t
        elif self.aggregate_func == "mean":
            update_r_t = functional.generalized_rspmm(adjacency, rel_time_input, input_r_t, sum="add", mul=mul)
            update_r_t = (update_r_t + boundary_r_t) / degree_out
        elif self.aggregate_func == "max":
            update_r_t = functional.generalized_rspmm(adjacency, rel_time_input, input_r_t, sum="max", mul=mul)
            update_r_t = torch.max(update_r_t, boundary_r_t)
        elif self.aggregate_func == "pna":
            sum = functional.generalized_rspmm(adjacency, rel_time_input, input_r_t, sum="add", mul=mul)
            sq_sum = functional.generalized_rspmm(adjacency, rel_time_input ** 2, input_r_t ** 2, sum="add", mul=mul)
            max = functional.generalized_rspmm(adjacency, rel_time_input, input_r_t, sum="max", mul=mul)
            min = functional.generalized_rspmm(adjacency, rel_time_input, input_r_t, sum="min", mul=mul)
            mean = (sum + boundary_r_t) / degree_out
            sq_mean = (sq_sum + boundary_r_t ** 2) / degree_out
            max = torch.max(max, boundary_r_t)
            min = torch.min(min, boundary_r_t)
            std = (sq_mean - mean ** 2).clamp(min=self.eps).sqrt()
            features = torch.cat([mean.unsqueeze(-1), max.unsqueeze(-1), min.unsqueeze(-1), std.unsqueeze(-1)], dim=-1)
            features = features.flatten(-2)
            scale = degree_out.log()
    ...
    def forward(self, graph, input_r_t):
        """
        ...
        """
        # input_r_t = torch.cat((input, input_t), dim=2)
        if self.gradient_checkpoint:
            update = checkpoint.checkpoint(self._message_and_aggregate, *graph.to_tensors(), input_r_t)
        else:
            # update = self.message_and_aggregate(graph, input, input_t)
            update = self.message_and_aggregate(graph, input_r_t)
        print("update.shape: " + str(update.shape))
        output = self.combine(input_r_t, update)
        return output
