...
class LogicalQuery(tasks.Task, core.Configurable):
    ...
    def preprocess(self, train_set, valid_set, test_set):
        ...
        # print(isinstance(dataset.fact_graph, data.Graph))
        # print(isinstance(dataset.fact_graph2, data.Graph))
        dataset.fact_graph.set_timestamp_vocab(dataset.timestamp_vocab)
        dataset.fact_graph.set_tid_vocab(dataset.tid_vocab)
        dataset.temporal_graph.set_timestamp_vocab(dataset.timestamp_vocab)
        dataset.temporal_graph.set_tid_vocab(dataset.tid_vocab)
        self.register_buffer("fact_graph", dataset.fact_graph)
        self.register_buffer("graph", dataset.temporal_graph)
    ...
    def predict_and_target(self, batch, all_loss=None, metric=None):
        ...
        if all_loss is None:
            ...
            # answer set cardinality prediction 答案集基数预测
            prob = F.sigmoid(pred)
    ...
    def batch_evaluate(self, pred, target):
        ...
        # unfiltered rankings of all answers 所有答案的未过滤排名
        ...
        # filtered rankings of all answers 所有答案的过滤排名
        ...
        # filtered rankings of hard answers hard答案的过滤排名
