...
class NeuralBellmanFordNetwork(nn.Module, core.Configurable):
    ...
    def __init__(self, input_dim, hidden_dims, num_relation, num_timestamp, message_func="distmult", aggregate_func="pna",
        ...
        num_timestamp = int(num_timestamp)
        ...
        for i in range(len(self.dims) - 1):
            self.layers.append(layer.GeneralizedRelationalConv(self.dims[i] * 2, self.dims[i + 1] * 2, num_relation,
                                                               self.num_timestamp, self.dims[0], message_func,
                                                               aggregate_func, layer_norm, activation, dependent))

    def forward(self, graph, input_r_t, all_loss=None, metric=None):
        with graph.node():
            # graph.boundary_r_t = torch.cat((input, input_t), dim=-1)
            graph.boundary_r_t = input_r_t
        hiddens = []
        # input_r_t = torch.cat((input, input_t), dim=2)
        layer_input = input_r_t
        ...
        node_query_r_r = graph.query_r_t.expand(graph.num_node, -1, -1)
        if self.concat_hidden:
            node_feature = torch.cat(hiddens + [node_query_r_r], dim=-1)
        else:
            node_feature = torch.cat([hiddens[-1], node_query_r_r], dim=-1)
